...
**ADR-19 — Checks de CI requeridos: siempre disparados, trabajo real gateado por path internamente**
Descartado: (a) mantener `ci-backend.yml`/`ci-frontend.yml` con `paths: ['backend/**']`/`['frontend/**']` en el trigger, tal como TT-07 los dejó. Por qué se descarta: branch protection (TT-02) exige que los checks `backend`/`frontend` pasen, pero un check requerido que nunca se dispara (porque el `paths:` del trigger lo filtró) no cuenta como "pasó" — GitHub lo deja en "Esperando" indefinidamente, y el único desbloqueo es el bypass de un admin. En la práctica, cada PR que solo toca `docs/` (varios en esta sesión: ADR-17, el gotcha de TT-16) necesitó bypass, exactamente el atajo que TT-02 quería evitar. (b) quitar el `paths:` del trigger sin más, dejando correr `npm ci` + lint + build + test completos en cada PR aunque no haya nada que revisar — funciona pero desperdicia minutos de CI y hace más lento cada PR de solo-docs sin necesidad.
Solución adoptada: se quita `paths:` del trigger (el job siempre corre, así el check requerido siempre se reporta), y se agrega un step `dorny/paths-filter@v3` al inicio de cada job que detecta si `backend/**`/`frontend/**` cambió. Cada step de trabajo real (`npm ci`, lint, tests, build, audit) queda condicionado con `if: steps.filter.outputs.<backend|frontend> == 'true'`. Resultado: un PR de solo-docs hace que el job corra y reporte éxito en segundos (sin pasos reales), satisfaciendo branch protection sin bypass; un PR que sí toca `backend/`/`frontend/` corre el pipeline completo como siempre. `dorny/paths-filter` ya queda cubierto por el `github-actions` ecosystem existente en `dependabot.yml` (sin cambios necesarios ahí).

**ADR-20 — Control de concurrencia en `LocationStock`: locking optimista con reintento acotado**
Descartado: (a) `SELECT ... FOR UPDATE` (locking pesimista) dentro de la transacción de Prisma — bloquea la fila hasta el commit, más caro bajo Postgres serverless con cold starts (Neon, TT-16) y no se justifica a esta escala (decenas/cientos de usuarios, conflictos reales infrecuentes); (b) devolver `409` inmediatamente en el primer conflicto, sin reintento automático — empeora la experiencia en el caso común (un conflicto aislado, no una carrera sostenida) trasladando al frontend un reintento que el backend puede resolver solo.
Solución adoptada: columna `version Int @default(0)` en `LocationStock` (migración `20260813180448_add_location_stock_version`), incrementada en cada `UPDATE` exitoso. El futuro use-case de movimientos (HU-08/HU-16/HU-17, todavía no implementado — Iteración 0) debe usar `updateMany({ where: { id, version }, data: { ..., version: { increment: 1 } } })`; si `count === 0` (otro proceso actualizó primero) se relee el stock y se reintenta desde cero, hasta 3 veces, antes de devolver `409 Conflict` al cliente. Utilidad genérica reusable en `backend/src/common/utils/optimistic-lock.util.ts` (`withOptimisticLock()`), cubierta con TDD (lógica de dominio crítica, ver `convenciones.md`) — sin consumidor real todavía porque ningún use-case de negocio existe aún, queda lista para cuando se implemente la primera HU que actualiza stock.

**ADR-21 — Idempotencia en escrituras críticas: constraint único en Postgres, no Redis**
Descartado: (a) deshabilitar el botón de submit en el frontend como única protección — es UX, no una garantía (mismo principio que RBAC, nunca confiar solo en el frontend); (b) Redis con TTL para las claves de idempotencia — típico en sistemas de mayor escala, pero agrega una pieza de infraestructura nueva (fuera de la lista de restricciones técnicas del TRD: "sin cache/Redis, no planeado para el MVP") que Postgres ya resuelve con un constraint único.
Solución adoptada: tabla `IdempotencyKey` (`key` único, `endpoint`, `response` como `Json`, `createdAt`) — migración `20260813181409_add_idempotency_key`. Patrón reusable: decorador `@Idempotent()` (`backend/src/common/decorators/idempotent.decorator.ts`) marca un endpoint como crítico; `IdempotencyInterceptor` (`backend/src/common/interceptors/`) exige el header `Idempotency-Key`, devuelve la respuesta ya guardada si la key ya existe (sin re-ejecutar el handler), y maneja la carrera de dos requests concurrentes con la misma key (constraint único de Postgres, código `P2002`) devolviendo la respuesta que ganó la inserción en vez de fallar. Cubierto con 5 tests unitarios (mock de `PrismaService`/`Reflector`/`ExecutionContext`). Sin consumidor real todavía (Iteración 0); queda listo para HU-08, HU-13, HU-15, HU-16.

**ADR-22 — Borrado lógico: `deletedAt` solo donde una HU real lo necesite, nunca preventivo**
Origen: hallazgo durante TT-20 (revisión de índices), formalizado como TT-22. Descartado: (a) generalizar `deletedAt DateTime?` a los 17 modelos de una sola vez — la mayoría (`RolePermission`, `ApprovalFlow`, `AuditEvent`, `InventoryMovement`, `LocationStock`, `PurchaseItem`, `RequestItem`) son tablas de enlace, configuración o el ledger inmutable (ADR-07); no tienen un ciclo de vida propio que un usuario "borre" y agregarles el campo sería trabajo especulativo sin HU que lo consuma; (b) seguir extendiendo el patrón `status` (enum `active`/`inactive`/…) caso por caso para todo lo que falte — mezcla dos conceptos distintos: `status` es estado de negocio (ej. un producto descontinuado sigue siendo un producto real, con historial), mientras que un borrado lógico responde a "el usuario pidió eliminar este registro" — forzar todo por `status` no distingue ambos casos cuando sí importa (ej. un proveedor "eliminado por error" vs. uno legítimamente inactivo).
Decisión: cuando la primera HU real defina una acción de "eliminar" sobre un modelo que no tenga ya `status` (`User`/`Location`/`Product`/`Supplier` se quedan como están, no se toca nada en ellos), se le agrega `deletedAt DateTime?` a ese modelo específico, y las lecturas normales lo excluyen vía una extensión de Prisma Client (`$extends`) que filtra automáticamente `deletedAt: null` en `findMany`/`findFirst`/`count` — no `where: { deletedAt: null }` repetido a mano en cada use-case (mismo espíritu que TT-19: no reimplementar el patrón compartido en cada lugar). Nunca se aplica a `InventoryMovement` ni `AuditEvent` (son el registro histórico, ADR-07) ni a tablas de enlace/config (`RolePermission`, `ApprovalFlow`) ni a filas derivadas sin ciclo de vida propio (`LocationStock`, `PurchaseItem`, `RequestItem` — se borran/quedan huérfanas junto con su padre, no individualmente). No se implementa nada de esto ahora — es una decisión de arquitectura para cuando aparezca la primera HU que la necesite, no una tarea de código de esta iteración.
