...
export class UpdateSupplierUseCase {
  ...
  async execute(id: string, dto: UpdateSupplierDto): Promise<SupplierResponseDto> {
    ...
    // be an active supplier with that (documentTypeId, taxId) pair — so it
    // fires when taxId changes, when documentTypeId changes, and when
    // status flips (back) to active, but never when the change is a
    // deactivation (moving out of the active pool can't create a new
    // conflict).
    if (dto.taxId !== undefined || dto.documentTypeId !== undefined || dto.status !== undefined) {
      const effectiveTaxId = dto.taxId !== undefined ? dto.taxId : existing.taxId;
      const effectiveDocumentTypeId =
        dto.documentTypeId !== undefined ? dto.documentTypeId : (existing.documentTypeId ?? undefined);
      const effectiveStatus = dto.status ?? existing.status;
      if (effectiveTaxId && effectiveStatus === 'active') {
        const duplicate = await this.supplierRepository.findActiveByTaxId(effectiveTaxId, effectiveDocumentTypeId);
