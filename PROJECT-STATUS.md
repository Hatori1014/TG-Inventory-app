...
> Última actualización: TT-17 (locking optimista), TT-18 (idempotencia) y TT-22 (decisión de borrado lógico, ADR-22) implementadas y con PR abiertos, **pendientes de merge** — con esto se completan las nueve tareas técnicas de la revisión de arquitectura (las ocho originales, TT-14 a TT-21, más TT-22). No se ha escrito ninguna HU de negocio todavía; el siguiente paso natural es empezar con la primera HU real una vez estos PR se mergeen. Actualízalo tú mismo al cerrar cada iteración. English version below.
...
### Próximo paso inmediato: mergear TT-17/TT-18/TT-22, luego arrancar la primera HU

TT-02 a TT-05 ya están cerradas (ver tabla abajo). Las nueve tareas técnicas de la revisión de arquitectura (ver "Gaps de arquitectura" abajo) están todas implementadas y verificadas localmente; faltan solo los merges:
- PR TT-17: `feature/tt-17-optimistic-locking` → `staging`
- PR TT-18: `feature/tt-18-idempotency` → `staging` (ramificado sobre TT-17 — mergear TT-17 primero para que el diff quede limpio)
- PR TT-22: `docs/tt-22-soft-delete-strategy` → `staging` (ramificado sobre TT-18 — mergear en ese orden)

Una vez mergeados los tres, no queda ningún prerrequisito técnico pendiente para empezar la primera HU de negocio real.
...
https://trello.com/b/BS5tzENy/sistema-de-control-de-inventario — incluye TT-14 a TT-22 en la lista `Iteración actual` (TT-14/15/16/19/20/21 ya movidas a `Hecho`, con evidencia en su descripción; TT-17/18/22 se moverán a `Hecho` con su evidencia una vez se confirmen los merges), además de las tarjetas originales TT-02 a TT-10.

---

# Project status — read this first

> Last updated: TT-17 (optimistic locking), TT-18 (idempotency), and TT-22 (soft-delete decision, ADR-22) implemented with open PRs, **pending merge** — this completes all nine technical tasks from the architecture review (the original eight, TT-14 through TT-21, plus TT-22). No business story has been implemented yet; the natural next step is starting the first real story once these PRs merge. Keep this updated yourself as each iteration closes.
...
### Immediate next step: merge TT-17/TT-18/TT-22, then start the first story

TT-02 through TT-05 are now closed (see table below). All nine technical tasks from the architecture review (see "Architecture gaps" below) are implemented and verified locally; only the merges remain:
- TT-17 PR: `feature/tt-17-optimistic-locking` → `staging`
- TT-18 PR: `feature/tt-18-idempotency` → `staging` (branched on top of TT-17 — merge TT-17 first so the diff stays clean)
- TT-22 PR: `docs/tt-22-soft-delete-strategy` → `staging` (branched on top of TT-18 — merge in that order)

Once all three are merged, no technical prerequisite is left before starting the first real business story.
...
| TT-17 (optimistic locking on stock) | 🟡 Implemented, PR open (`feature/tt-17-optimistic-locking`), pending merge — `version Int @default(0)` column on `LocationStock` (migration `20260813180448_add_location_stock_version`, a single `ALTER TABLE ADD COLUMN`, reviewed line by line), reusable `withOptimisticLock()` utility (`backend/src/common/utils/optimistic-lock.util.ts`): retries up to 3 times on a version conflict, then throws `409 Conflict`. Decision documented in **ADR-20** (discards `SELECT FOR UPDATE` and 409-with-no-retry). Covered with 4 TDD unit tests. No real consumer yet (Iteration 0, no movements use-case implemented) — ready for HU-08/HU-16/HU-17. `npm run lint`/`build`/`test` green (5 suites, 16 tests) at commit time. **Not applied to Neon staging yet** — will be applied along with the rest after merge, following TT-20's same mechanism (`npm run prisma:migrate:staging`) |
| TT-18 (idempotency) | 🟡 Implemented, PR open (`feature/tt-18-idempotency`), pending merge (branched on top of TT-17 — merge TT-17 first) — `IdempotencyKey` table (unique `key`, `endpoint`, `response` as `Json`, `createdAt`; migration `20260813181409_add_idempotency_key`, a single `CREATE TABLE` + unique index), reusable `@Idempotent()` decorator + `IdempotencyInterceptor` (`backend/src/common/`): requires the `Idempotency-Key` header, returns the already-stored response without re-running the handler when the key repeats, and resolves the race between two concurrent requests sharing a key via Postgres' `P2002` code (unique constraint), returning whichever response won instead of failing. Decision documented in **ADR-21** (discards Redis, disabling the frontend button). Covered with 5 unit tests (mocked `PrismaService`/`Reflector`/`ExecutionContext`, including the race case). No real consumer yet; ready for HU-08/HU-13/HU-15/HU-16. `npm run lint`/`build`/`test` green (6 suites, 21 tests) at commit time. **Not applied to Neon staging yet** |
| TT-22 (soft delete: strategy and scope) | 🟡 Decision made and documented, PR open (`docs/tt-22-soft-delete-strategy`), pending merge (branched on top of TT-18) — **ADR-22**: `deletedAt DateTime?` gets added model by model, only once the first real story defines a "delete" action on a model without an existing `status` (`User`/`Location`/`Product`/`Supplier` untouched); reads filtered via a Prisma Client extension (`$extends`), not a hand-repeated `where: { deletedAt: null }`. Never applies to `InventoryMovement`/`AuditEvent` (historical record, ADR-07) nor to link/config tables or derived rows with no lifecycle of their own (`RolePermission`, `ApprovalFlow`, `LocationStock`, `PurchaseItem`, `RequestItem`). No code or schema changes — this is exactly what the Trello card itself asks for (apply the pattern only once the first story needs it, not preemptively) |
...
https://trello.com/b/BS5tzENy/sistema-de-control-de-inventario — includes TT-14 through TT-22 in the `Current iteration` list (TT-14/15/16/19/20/21 already moved to `Done`, with evidence in their description; TT-17/18/22 will move to `Done` with their evidence once the merges are confirmed), plus the original TT-02 through TT-10 cards.
